...
\begin{frame}{One Reservoir Choice: Echo State Networks}
  \begin{itemize}
  \item what system to use for reservoir?
    \vspace{2em}
  \item One common choice: use an \emph{untrained} neural network as reservoir
  ...
  \item network is (usually) \emph{recurrent}
  \end{itemize}
  \centering
  \begin{align*}
...
\begin{frame}{Echo State Network Overview}
  \centering
    ...
    \begin{columns}
      ...
      \item looking for ESNs that work well at forecasting Lorenz~'63 system
        \begin{equation*}
          ...
          \end{aligned}
        \end{equation*}
        with $\sigma = 10$, $\rho = 28$, and $\beta = 8/3$.\footnote[frame]{\fullcite{lorenz1963}}
      \end{itemize}
      \column{0.5\textwidth}
      \centering
...
\begin{frame}{Training, Forecasting, and Evaluation}
  ...
  \item total 100 trial parameters for each structure
...
\begin{frame}{Practical Considerations}
  ...
  \item \emph{Why ever use an ESN if an NVAR will work?}
    \vspace{2em}
  \item for $q$ taps, $d$ input dimensions, and $n$-degree polynomial terms, $W_\text{out}$ has to fit $(qd)^n$ components (expensive!)
