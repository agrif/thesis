...
The NVAR also occasionally exhibits fixed points that do not have a
corresponding fixed point in the Lorenz system. These spurious fixed
points depend on which specific segment of Lorenz data is used to
train the NVAR, and will move around or disappear as the training data
changes. Future research could identify how many of these spurious
fixed points exist, as well as how to prevent them or otherwise
control for their presence.
...
I estimate the speed-up by summing the largest contributions for the
...
To generalize well after training, I increase the ridge parameter
...
Unlike RCs, the weight matrix $W_\text{out}$ of a trained NVAR is
